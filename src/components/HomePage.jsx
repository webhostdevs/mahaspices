...
const Homepage = () => {
  ...
  {menuBoxes.map((box) => (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 w-full h-1/3 bg-gradient-to-t from-black/70 to-transparent p-5 flex flex-col justify-end transition-opacity duration-300 ease-in-out">
        <h3 className="text-xl sm:text-2xl font-bold text-white mb-2">{box.title}</h3>
        <button className="inline-block px-4 sm:px-5 py-2 sm:py-3 bg-green-600 text-white text-sm sm:text-base rounded-full hover:bg-green-700 transition">
          Know More
        </button>
      </div>
    </motion.div>
